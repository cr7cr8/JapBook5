...
function RightAction({ progress, drag, panel, sourceWord, index, visiblePanel, ...props }) {
    ...
    const { autoPlay, isListPlaying, vibrate, deleteDownloadWord, setRefreshState, frameTransY, lightOrDarkstate, toggleLightOrDark } = useContext(Context)
    ...
    const iconContainnerStyle = useAnimatedStyle(() => {
        ...
        return {
            ...
            backgroundColor: frameTransY.value >= 160 ? D6BD95 : e7cca0,//"#D6BD95" : "#e7cca0",

            borderWidth: 0,
            borderBottomWidth: 1,
            borderBottomColor: wheat == "wheat" ? "#D2B48C" : "#555",
        }
...
function LeftAction({ progress, drag, panel, sourceWord, index, visiblePanel, ...props }) {
    ...
    const { autoPlay, isListPlaying, frameTransY, setSource, sourceWordArr, setSouceWordArr, saveWordToFile, isNewerstOnTop, setRefreshState, lightOrDarkstate, toggleLightOrDark } = useContext(Context)
    const styleAnimation = useAnimatedStyle(() => {
...
export default function SwipebleRowItem(props) {
    ...
    const { sourceWordArr, scrollRef, isListPlaying, sharedStatus, lightOrDarkstate, toggleLightOrDark } = useContext(Context)
    const {
        "#D6BD95": D6BD95,
        "#e7cca0": e7cca0,
        "#a75d09": a75d09,
...
export function PanelItem({ sourceWord, index, visiblePanel, panelRef, ...props }) {
    ...
    const { sourceWordArr, scrollRef2, wordPos, frameTransY, shouldFrameDisplay, isListPlaying,
        ...
        refreshState, setRefreshState, isManualDrag, lightOrDarkstate, toggleLightOrDark
    } = useContext(Context)
            ...
            <GestureDetector gesture={Gesture.Simultaneous(
                ...
                <View style={[panelStyle]} >
                    ...
                    <View style={[focusedPanelStyle]}>
                        <LinearGradient
                            colors={wheat === "wheat"
                                ? ["rgba(255, 179, 80, 0)", 'rgba(255, 179, 80, 0.8)', 'rgba(255, 155, 133, 1)',]
                                // : ["rgba(255, 179, 80, 0.1)", 'rgba(255, 179, 80, 0.3)', 'rgba(255, 155, 133, 0.5)',]
                                : ["rgba(255, 179, 80, 0)", 'rgba(255, 179, 80, 0.3)', 'rgba(255, 155, 133, 0.5)',]
                            }
                            dither={true}
                            start={{ x: 0, y: 1 }}
                            end={{ x: 1, y: 1 }}
