...
export default function Card(props, dim) {
    ...
    const { sourceWordArr, frameTransY, shouldFrameDisplay, scrollRef2, isCardMoving, isScrollingY, isScrollingX, isListPlaying, speak, scrollX,
        downloadWord, refreshState, wordPos, lightOrDarkstate, toggleLightOrDark
    ...
    useEffect(() => {
        ...
        const exists = new File(Paths.document, `${hashName + hashName}.mp3`).exists
...
function WordPanel({ sourceWord, index, visibleCard }) {
    ...
    const { sourceWordArr, frameTransY, isCardMoving, downloadWord, deleteDownloadWord, refreshState, isScrollingY, isScrollingX, stopSpeak, autoPlay,
        scrollRef2Scroll, wordPos, isListPlaying, scrollY, scrollX, speak, checkPlaying, lightOrDarkstate, toggleLightOrDark
    ...
    useEffect(() => {
        ...
        isDownloaded.value = new File(Paths.document, `${hashName1 + hashName2}.mp3`).exists
            ...
            <GestureDetector gesture={Gesture.Exclusive(
                ...
                <View style={useAnimatedStyle(() => {
                    return {
                        ...
                        minHeight: 80,
                        paddingHorizontal: 4,
                        backgroundColor: isDownloaded.value ? wheat : e7cca0,//"#e7cca0",
                        borderTopWidth: 2,
                ...
                >
                    ...
                    <Text style={{ fontSize: 27, color: a75d09,/*"#a75d09",*/ }} onLayout={(e) => {
                        //  e.persist()
                    ...
                    <Text ellipsizeMode={"tail"} style={{ position: "absolute", right: 0, fontSize: 15, transform: [{ translateY: 8 }], color: wheat == "wheat" ? "#555" : "#999" }}>
                        {index + " "}
                    </Text>

                    <Text style={{ fontSize: 17, color: wheat == "wheat" ? "#555" : "#ccc" }} onLayout={(e) => {
                        //  e.persist()
...
function SentenceArea({ index, sourceWord, children, wordRowHeight, visibleCard, ...props }) {
    const { sourceWordArr, frameTransY, isCardMoving, downloadWord, refreshState, isScrollingY, setRefreshState, isScrollingX, scrollX, speak, isListPlaying,
        checkPlaying, audioPlayer, stopSpeak, sentencePlayingIndex, wordPos, lightOrDarkstate, toggleLightOrDark
    } = useContext(Context)
    ...
    //                     if (sentencePlayingIndex.value > itemIndex) {
            ...
            <View
                ...
                style={useAnimatedStyle(() => {
                    ...
                    return {
                        ...
                        borderWidth: 0,
                        backgroundColor: D6BD95,//"#D6BD95",//"darkgray",//"lightblue",
                ...
                >
                    ...
                    <View
                        ...
                        style={useAnimatedStyle(() => {
                            ...
                            return {
                                display: "flex", backgroundColor: D6BD95,//  "#D6BD95",//"darkgray",// "purple",
                                width: screenWidth,
...
function SentencePanel({ index, arrIndex, visibleCard, ...props }) {
    const { sourceWordArr, frameTransY, isCardMoving, downloadWord, refreshState, isScrollingY, setRefreshState, deleteDownloadWord,
        isScrollingX, scrollX, speak, isListPlaying, checkPlaying, audioPlayer, stopSpeak, lightOrDarkstate, toggleLightOrDark } = useContext(Context)
    ...
    useEffect(() => {
        ...
        const sentenceFile = new File(Paths.document, `${hashName1 + hashName2}.mp3`)
        isDownloaded.value = sentenceFile.exists
    ...
    function startDownloadWord() {
        downloadWord(sourceWord.wordName, sourceWord.exampleEnglishArr[arrIndex].sentence, checkDownload)
    }
    function startDeleteDownloadWord() {
        deleteDownloadWord(sourceWord.wordName, sourceWord.exampleEnglishArr[arrIndex].sentence, checkDownload)
    }
            ...
            <GestureDetector gesture={Gesture.Exclusive(
                ...
                Gesture.LongPress()
                    .onStart(e => {
                        ...
                        if (!isDownloaded.value) {
                            scheduleOnRN(startDownloadWord)
                        }
                        else if (isDownloaded.value) {
                            scheduleOnRN(startDeleteDownloadWord)
                        }
                ...
                <View style={useAnimatedStyle(() => {
                    return {
                        ...
                        paddingTop: 0,
                        borderWidth: 0,
                        borderColor: isDownloaded.value ? wheat : e7cca0,
                        borderBottomWidth: 0
                ...
                >
                    <Text style={{ fontSize: 4, color: "#999", paddingBottom: 0 }} onLayout={(e) => {
                        //  e.persist()
                        //chineseHeight.value = e.nativeEvent.layout.height
                    }}>
                        {" "}    {/*  //to separator the upper and the lower sentence, use the padding or bottomwidth creates extra line. */}
                    </Text>

                    <Text style={{ fontSize: 25,  color: wheat == "wheat" ? "#111" : "#ccc"}} onLayout={(e) => {
                        // e.persist()
                        englishHeight.value = e.nativeEvent.layout.height
                    }}>
                    ...
                    <Text style={{ fontSize: 25,  color: wheat == "wheat" ? "#555" : "#888", paddingBottom: 0 }} onLayout={(e) => {
                        //  e.persist()
                        chineseHeight.value = e.nativeEvent.layout.height
                    }}>
