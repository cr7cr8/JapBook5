...
export default function DragListScreen() {
    ...
    const { sourceWordArr, setSouceWordArr, wordPos, scrollX, saveWordToFile, setRefreshState, lightOrDarkstate, toggleLightOrDark } = useContext(Context)
...
const {
    ...
    useEffect(() => {
        ...
        if (!Boolean(sourceWord?.wordName)) {
            return
        }
        const hashName1 = CryptoJS(sourceWord.wordName).toString()
